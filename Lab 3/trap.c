...
trap(struct trapframe *tf)
{
  ...
  //TODO5: Adding a case where we read in the register CR2 through rcr2(). 
  // We need to check the if the address in the register cr2 is fromt eh page reight under the current bottom of the stack. If so we need to grow the stack with allocuvm. 
  case T_PGFLT: ;
    ...
    break;
  case T_IRQ0 + IRQ_TIMER: //page grows into heap. 
    if(cpuid() == 0){
      acquire(&tickslock);
      ticks++;
      wakeup(&ticks);
